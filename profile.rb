# -*- encoding: utf-8 -*-
# original scraping source code at https://github.com/yatish27/linkedin-scraper
# Modified some code to extract the fields of interest
#
require "rubygems"
require "mechanize"
require "cgi"
...
module Linkedin
  class Profile
    ...
    def courses
	  #[{:school => 'mit', :courses => ['course1', 'course2']}]
	  schools ||= @page.search("#courses .item-title").map do |item|
        school = item.text.gsub(/\s+|\n/, " ").strip
	  end
	  
	  id = 0
      @courses ||= @page.search("#courses .courses-list").map do |course_list|
	    courses_at_school ||= course_list.search('li').map do |course|
          course_title = course.text.gsub(/\s+|\n/, " ").strip
		end
		id += 1
		{:school => schools[id -1], :courses => courses_at_school} if (id - 1) < schools.length
	  end
    end
...
#profile = Linkedin::Profile.get_profile("file:///home/bekaar/views/my_projects/linkedin_data/results/Anant_Agarwal_LinkedIn.html")
#profile.courses
